...
```{r covariates-table}
data_list$covariates_table %>%
  dplyr::mutate_all(linebreak, align='l') %>%
  knitr::kable(
    'latex', booktabs = T, escape = F, caption = captions$covariates_table,
    col.names = linebreak(colnames(data_list$covariates_table))
  ) %>%
...
All data sources used in this analysis are publicly available online. The code repository accompanying this chapter, available online at https://github.com/njhenry/covidemr, contains detailed instructions for downloading and formatting each data source.
...
```{r excess-ratio, out.width="100%", fig.cap = captions$excess_ratio}
knitr::include_graphics(graphics_fps$excess_ratio)
```
...
```{r age-excess, out.width="100%", fig.cap = captions$age_excess}
knitr::include_graphics(graphics_fps$age_excess)
```

```{r excess-demographics}
data_list$excess_demographics %>%
  knitr::kable(
    'latex', booktabs = T, escape = F, caption = captions$excess_demographics,
    col.names = linebreak(colnames(data_list$excess_demographics), align='c'), align = 'c'
  ) %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
...
```{r age-maps, out.width="100%", fig.cap = captions$age_maps}
...
```{r top-provinces, fig.pos='!p', out.height="85%", fig.cap = captions$top_provinces}
...
```{r excess-neighbors, fig.pos='H', out.width="100%", fig.cap = captions$excess_neighbors}
